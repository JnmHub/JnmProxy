...
import { batchNodes, checkAllNodes, checkNode, listNodes, rebuildNodeAdapter, setNodeEnabled, type NodeFilter } from '../api/nodes';
...
export function NodesPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader title="筛选" description="按订阅、协议、健康状态和名称过滤节点。" action={<Button onClick={() => checkAllMutation.mutate()}><RefreshCw className="h-4 w-4" />批量健康检查</Button>} />
        <div className="grid gap-3 md:grid-cols-5">
          ...
          <Field label="订阅"><Select value={filter.subscription_id ?? ''} onChange={(event) => setFilter({ ...filter, subscription_id: Number(event.target.value) || undefined })}><option value="">全部</option>{(subscriptionsQuery.data ?? []).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</Select></Field>
          <Field label="协议"><Input value={filter.protocol ?? ''} onChange={(event) => setFilter({ ...filter, protocol: event.target.value || undefined })} placeholder="vmess" /></Field>
          <Field label="健康"><Select value={filter.alive_status ?? ''} onChange={(event) => setFilter({ ...filter, alive_status: event.target.value as NodeFilter['alive_status'] })}><option value="">全部</option><option value="alive">可用</option><option value="dead">死亡</option><option value="unknown">未知</option></Select></Field>
          <Field label="启用"><Select value={filter.enabled === '' || filter.enabled === undefined ? '' : String(filter.enabled)} onChange={(event) => setFilter({ ...filter, enabled: event.target.value === '' ? '' : event.target.value === 'true' })}><option value="">全部</option><option value="true">启用</option><option value="false">禁用</option></Select></Field>
        </div>
      </Card>
      <Card>
        <CardHeader title="节点列表" description={`当前 ${nodes.length} 个节点，已选 ${selected.length} 个。`} action={<BatchActions selected={selected} groups={groupsQuery.data ?? []} onBatch={(action, groupID) => batchMutation.mutate({ action, groupID })} />} />
        <DataTable columns={['选择', '名称', '协议', '健康', 'sing-box', '延迟', '服务器', '操作']} empty={!nodes.length}>
          {nodes.map((node) => (
            <tr key={node.id} className="hover:bg-slate-900/50">
              ...
              <td className="px-4 py-3"><div className="flex flex-wrap gap-2"><Button onClick={() => checkMutation.mutate(node.id)}>检查</Button><Button onClick={() => setEnabledMutation.mutate({ id: node.id, enabled: !node.enabled })}>{node.enabled ? '禁用' : '启用'}</Button><Button onClick={() => rebuildMutation.mutate(node.id)}><RotateCcw className="h-4 w-4" />重建</Button></div></td>
            </tr>
          ))}
...
function BatchActions({ selected, groups, onBatch }: { selected: number[]; groups: Array<{ id: number; name: string }>; onBatch: (action: 'enable' | 'disable' | 'add_group' | 'remove_group', groupID?: number) => void }) {
